...
func (r *PluginRunner) Run(ctx context.Context, pluginSets map[int]plugin.PluginSet, hs plugin.HandshakeConfig, env []string, logger log.Logger) (*plugin.Client, error) {
	return r.runCommon(ctx, pluginSets, hs, env, logger)
}

func (r *PluginRunner) runCommon(ctx context.Context, pluginSets map[int]plugin.PluginSet, hs plugin.HandshakeConfig, env []string, logger log.Logger) (*plugin.Client, error) {
...
func LookBackendExecute(directory, name string) bool {
	pName := fmt.Sprintf("%s/backend.goplugin", name)
