...
func NewPluginManager(directory string,
	consul consul.Client,
	params logical.Encodable,
	logger hclog.Logger) *PluginManager {

	return &PluginManager{
		logger:    logger.Named("goplugin-manager"),
		rwMutex:   sync.RWMutex{},
		directory: directory,
		backends:  map[string]*Backend{},
		consul:    consul,
...
func (p *PluginManager) Deregister(name string) error {
	...
	if backend.pluginType != PluginTypeLogical {
		return errors.New("goplugin deregister unsupported")
	}
...
func (p *PluginManager) start(pluginEntry PluginEntry) error {
	...
	conf := &logical.BackendConfig{
		...
		ConsulView:  logical.NewConsulView(p.consul),
	}
...
func (p *PluginManager) newBuiltinBackend(conf *logical.BackendConfig) (logical.Backend, error) {
	return goplugin.NewBackend(context.Background(), p.registry, conf, false)
}
